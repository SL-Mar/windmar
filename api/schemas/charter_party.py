...
class LegWeatherInput(BaseModel):
    ...
    wave_height_m: float = Field(0, ge=0, description="Significant wave height in metres")
...
class GoodWeatherRequest(BaseModel):
    ...
    bf_threshold: int = Field(4, ge=0, le=12, description="Max Beaufort force for good weather")
    wave_threshold_m: Optional[float] = Field(None, ge=0, description="Max wave height (m)")
    current_threshold_kts: Optional[float] = Field(None, ge=0, description="Max current speed (kts)")
...
class WarrantyVerificationRequest(BaseModel):
    ...
    voyage_id: str = Field(..., min_length=1, description="Voyage UUID")
    warranted_speed_kts: float = Field(..., gt=0, le=30, description="Chartered speed (kts)")
    warranted_consumption_mt_day: float = Field(..., gt=0, description="Chartered daily consumption (MT/day)")
    bf_threshold: int = Field(4, ge=0, le=12)
    speed_tolerance_pct: float = Field(0, ge=0, le=20, description="Allowable speed deficit %")
    consumption_tolerance_pct: float = Field(0, ge=0, le=20, description="Allowable consumption excess %")
...
class OffHireRequest(BaseModel):
    ...
    rpm_threshold: float = Field(10, ge=0, description="RPM below which engine is stopped")
    speed_threshold: float = Field(1, ge=0, description="Speed (kts) below which vessel is drifting")
